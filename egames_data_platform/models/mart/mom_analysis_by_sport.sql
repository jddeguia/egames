...
mom_comparison AS (
    SELECT
        *,
        LAG(total_turnover) OVER (PARTITION BY sport ORDER BY year, month_number) AS prev_month_turnover,
        LAG(total_profit) OVER (PARTITION BY sport ORDER BY year, month_number) AS prev_month_profit
    FROM monthly_data
)
