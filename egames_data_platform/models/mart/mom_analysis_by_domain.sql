...
WITH monthly_data AS (
    ...
    FROM {{ ref('stg_egames') }}
    GROUP BY domain, year, month_number
),

mom_comparison AS (
    SELECT
        *,
        LAG(total_turnover) OVER (PARTITION BY domain ORDER BY year, month_number) AS prev_month_turnover,
        LAG(total_profit) OVER (PARTITION BY domain ORDER BY year, month_number) AS prev_month_profit
    FROM monthly_data
)
