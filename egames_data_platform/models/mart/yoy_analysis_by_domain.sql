...
yoy_comparison AS (
    SELECT
        *,
        LAG(total_turnover) OVER (PARTITION BY domain ORDER BY year) AS prev_year_turnover,
        LAG(total_profit) OVER (PARTITION BY domain ORDER BY year) AS prev_year_profit
    FROM yearly_data
)
